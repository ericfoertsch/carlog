...
<template>
  <div class="car-card" @click="$emit('select', car.id)">
    ...
    <div class="car-info">
      ...
      <div v-if="car.body || car.color || car.engine || car.photos.length > 1" class="car-tags">
        <span v-for="tag in [car.body, car.color, car.engine].filter(Boolean).slice(0,3)" :key="tag" class="tag">{{ tag }}</span>
